function convert2plan(app)
        ...
        for i = 1:size(app.ProcessedData.YawGimbalAngle, 1)-1
            % Collect Gimbal Angle from Main Data
        gimbalYaw = app.ProcessedData.YawGimbalAngle(i);
        ...
        heading = mod(90 - heading, 360);
            ...
            if heading > 180 && heading < 270
                app.ProcessedData.YawGimbalAngle(i) = gimbalYaw;
            elseif heading > 0 && heading < 90
                app.ProcessedData.YawGimbalAngle(i) = -gimbalYaw;
            elseif heading > 90 && heading < 180
                app.ProcessedData.YawGimbalAngle(i) = -180;
            end
        ...
        % --- Creating .plan file --------------------------------------
            ...
            % Set a default home position (set take-off point)
